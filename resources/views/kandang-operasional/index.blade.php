                            ...
                            <div class="p-5">
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        ...
                                        <h3 class="text-lg font-bold text-gray-900 leading-tight">{{ $batch->nama_batch }}</h3>
                                    </div>
                                ...
                                <div class="space-y-2 text-sm text-gray-600 mb-6">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-medium text-gray-900">{{ $batch->jenis_ayam }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span>Supplier:</span>
                                        <span class="font-medium text-gray-900">{{ $batch->supplier->nama_supplier ?? '-' }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span>Tanggal Masuk:</span>
                                        <span class="font-medium text-gray-900">{{ \Carbon\Carbon::parse($batch->tgl_masuk)->translatedFormat('d M Y') }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span>Umur Masuk:</span>
                                        <span class="font-medium text-gray-900">{{ $batch->umur_awal_minggu }} Minggu</span>
                                    </div>
                                    <div class="flex justify-between border-t border-orange-100 pt-2 mt-2">
                                        <span class="font-bold text-gray-700">Sisa Belum Assign:</span>
                                        <span class="font-bold text-red-600 text-base">{{ number_format($batch->jumlah_sisa, 0, ',', '.') }} Ekor</span>
                                    </div>
                                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                @forelse($kandangs as $kandang)
                    @php
                        ...
                        $bgGaugeColor = 'bg-emerald-100';
                        if ($percentage >= 90) {
                            $gaugeColor = 'bg-red-500';
                            $bgGaugeColor = 'bg-red-100';
                        } elseif ($percentage >= 70) {
                            $gaugeColor = 'bg-amber-500';
                            $bgGaugeColor = 'bg-amber-100';
                        }
                    @endphp

                    <x-card class="border border-gray-200 flex flex-col h-full overflow-hidden">
                        <div class="p-5 flex-grow">
                            <div class="flex justify-between items-start mb-2">
                                <h3 class="text-xl font-bold text-gray-900">{{ $kandang->nama_kandang }}</h3>
                                <div class="text-right">
                                    <span class="block text-2xl font-black text-gray-900">{{ number_format($kandang->populasi_saat_ini, 0, ',', '.') }}</span>
                                    <span class="block text-xs text-gray-500">Total Ekor</span>
                                </div>
                            </div>
                            
                            <p class="text-sm text-gray-500 mb-4">Kapasitas Maksimal: {{ number_format($kandang->kapasitas_kandang, 0, ',', '.') }} ekor</p>

                            {{-- Gauge Bar --}}
                            <div class="mb-2 flex justify-between text-xs font-bold">
                                <span class="text-gray-700">Kepenuhan</span>
                                <span class="{{ $percentage >= 90 ? 'text-red-600' : ($percentage >= 70 ? 'text-amber-600' : 'text-emerald-600') }}">{{ $percentage }}%</span>
                            </div>
                            ...
                            <div x-data="{ expanded: false }" class="border border-gray-100 rounded-lg overflow-hidden">
                                <button @click="expanded = !expanded" class="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 flex justify-between items-center text-sm font-bold text-gray-700 transition-colors">
                                ...
                                <div x-show="expanded" x-collapse>
                                    <div class="divide-y divide-gray-100">
                                        ...
                                        @empty
                                            <div class="p-3 bg-white text-sm text-center text-gray-500 italic">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500 mb-4">
                                            Tempatkan ayam muda (pullet) dari <span class="font-bold text-gray-900" x-text="selectedBatch ? selectedBatch.nama_batch : ''"></span> ke kandang produksi.
                                        </p>
                                        ...
                                        <div class="space-y-4">
                                            <div>
                                                <label for="id_kandang" class="block text-sm font-medium text-gray-700 mb-1">Pilih Kandang Target <span class="text-red-500">*</span></label>
                                                <select name="id_kandang" id="id_kandang" x-model="selectedKandangId" @change="checkCapacity" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm">
                                                ...
                                                <p x-show="selectedKandangId && sisaKapasitasTarget !== null" class="mt-1 text-xs" :class="sisaKapasitasTarget <= 0 ? 'text-red-600 font-bold' : 'text-blue-600'">
                                                    Sisa Kapasitas Kandang: <span x-text="sisaKapasitasTarget"></span> ekor
                                                </p>
                                            </div>

                                            <div>
                                                <label for="jumlah" class="block text-sm font-medium text-gray-700 mb-1">Jumlah Ayam yang Ditempatkan <span class="text-red-500">*</span></label>
                                                <input type="number" name="jumlah" id="jumlah" x-model.number="assignAmount" min="1" :max="selectedBatch ? selectedBatch.jumlah_sisa : 1" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm" placeholder="Contoh: 1000">
                                                
                                                <p x-show="isOverCapacity" class="mt-1 text-xs text-red-600 font-bold flex items-center">
                                                ...
                                                <p x-show="isOverBatch" class="mt-1 text-xs text-red-600 font-bold flex items-center">
                                                    <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
