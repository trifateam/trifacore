...
@section('content')
    ...
    <x-page-header title="Assign Pullet ke Kandang" subtitle="Tempatkan ayam muda (pullet) dari {{ $batch->nama_batch ?? $batch->kode_batch }} ke kandang produksi." />

    <div class="max-w-3xl mx-auto mt-6" x-data="assignKandang()">
        <x-card class="border border-gray-200 dark:border-gray-700">
            ...
            <div class="p-5 grid grid-cols-2 md:grid-cols-3 gap-4 border-b border-gray-200 dark:border-gray-700">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Nama / Kode Batch</p>
                    <p class="mt-1 text-sm font-bold text-gray-900 dark:text-gray-100">{{ $batch->nama_batch ?? $batch->kode_batch }}</p>
                </div>
                <div>
            ...
            <form method="POST" action="{{ route('batch.assign', $batch->id_batch) }}">
                ...
                <div class="p-5 space-y-6">
                    ...
                    <div class="bg-orange-50 dark:bg-orange-900/30 p-4 rounded-lg border border-orange-100 dark:border-orange-800 text-sm flex justify-between items-center">
                        <span class="text-orange-800 dark:text-orange-300 font-medium">Seluruh Populasi Batch Ini Akan Ditempatkan:</span>
                        <span class="font-bold text-orange-900 dark:text-orange-100 text-xl">{{ number_format($batch->populasi_saat_ini, 0, ',', '.') }} Ekor</span>
                    </div>
                        ...
                        <div>
                            ...
                            <select name="id_kandang" id="id_kandang" x-model="selectedKandangId" @change="checkLimit" required class="w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                                <option value="">-- Pilih Kandang --</option>
                                @foreach($kandangs as $k)
                                    <option value="{{ $k->id_kandang }}">{{ $k->nama_kandang }} (Populasi Saat Ini: {{ $k->populasi_saat_ini }}) - {{ $k->active_batch_count }} Batch Aktif</option>
                                @endforeach
                            </select>
                            
                            <p x-show="isOverBatchLimit" class="mt-2 text-sm text-red-600 dark:text-red-500 font-bold flex items-center">
                                ...
                                Kandang sudah mencapai batas maksimal 2 Batch Aktif!
                            </p>
                        </div>
    ...
    function assignKandang() {
        return {
            ...
            checkLimit() {
                ...
                if(k) {
                    this.activeBatchCount = k.active_batch_count;
                }
            },

            get isOverBatchLimit() {
                return this.selectedKandangId && this.activeBatchCount >= 2;
            },

            get isSubmitDisabled() {
                return !this.selectedKandangId || this.isOverBatchLimit;
            }
        }
    }
</script>
