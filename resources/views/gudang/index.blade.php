...
@section('content')
    ...
    <div x-data="gudangPage()" class="mt-6 space-y-6">
        ...
        @if($countKritis > 0 || $countHabis > 0)
            <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-md flex items-start shadow-sm">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-bold text-red-800">Peringatan Persediaan Barang!</h3>
        ...
        <x-card class="border border-gray-200">
            <div class="p-5">
                <form method="GET" action="{{ route('gudang.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Cari Nama Barang</label>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cth: Pakan Starter..." class="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Kategori Barang</label>
                        <select name="kategori" class="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="flex gap-2">
                        ...
                        <div class="flex-1">
                            <a href="{{ route('gudang.index') }}" class="w-full h-9 px-4 inline-flex items-center justify-center border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 transition-colors">Reset</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <x-card class="border border-gray-200">
            <x-table :headers="['No', 'Nama Barang', 'Kategori', 'Stok Saat Ini', 'Satuan', 'Stok Minimum', 'Status', 'Aksi']">
                @forelse($paginatedBarang as $index => $barang)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $paginatedBarang->firstItem() + $index }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ $barang->nama_barang }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $barang->kategori_barang }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ number_format($barang->stok_barang, 2, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $barang->satuan }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($barang->stok_minimum, 2, ',', '.') }}</td>
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <div class="font-bold text-gray-900 bg-gray-50 p-2 rounded border border-gray-200" x-text="selectedItemName"></div>
                                        </div>
                                        
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label class="block text-xs font-medium text-gray-500 mb-1">Stok Sistem Saat Ini</label>
                                            ...
                                            <div>
                                                <label for="stok_fisik" class="block text-xs font-medium text-gray-700 mb-1">Stok Fisik Aktual <span class="text-red-500">*</span></label>
                                                <div class="relative">
                                                    <input type="number" step="0.01" name="stok_fisik" id="stok_fisik" x-model.number="inputStokFisik" min="0" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm font-bold">
                                                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                        <span class="text-gray-500 sm:text-xs uppercase" x-text="selectedSatuan"></span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <p x-show="isSameStock" class="text-xs font-bold text-amber-600 flex items-center bg-amber-50 p-2 rounded">
                                        ...
                                        <div>
                                            <label for="alasan" class="block text-xs font-medium text-gray-700 mb-1">Alasan Penyesuaian <span class="text-red-500">*</span></label>
                                            <textarea name="alasan" id="alasan" x-model="inputAlasan" rows="3" required maxlength="255" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: Barang tumpah/rusak, koreksi perhitungan bulan lalu..."></textarea>
                                        </div>
                                    </div>
