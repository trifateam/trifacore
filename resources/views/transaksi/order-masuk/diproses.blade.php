...
@section('content')
    ...
    <x-page-header title="Order Diproses" subtitle="Order penjualan yang sedang disiapkan barangnya" />
        ...
        @if($orders->isEmpty())
            <x-card class="p-8 text-center border border-gray-200 dark:border-gray-700">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-semibold text-gray-500 dark:text-gray-400">Tidak ada order yang sedang diproses</p>
                </div>
            </x-card>
        @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tgl Order</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tgl Proses</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($orders as $order)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $order->tanggal_proses ? \Carbon\Carbon::parse($order->tanggal_proses)->translatedFormat('d M Y H:i') : '-' }}</td>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                                        ...
                                        @if($order->latitude && $order->longitude)
                                            <a href="https://maps.google.com/?q={{ $order->latitude }},{{ $order->longitude }}" target="_blank" class="mt-1 inline-flex items-center text-xs font-medium text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-1">
                                            ...
                                            {{-- Download PDF --}}
                                            <a href="{{ route('transaksi.order-masuk.surat-jalan.pdf', $order->id_penjualan) }}" class="p-1.5 rounded-md text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 transition-colors" title="Download Surat Jalan PDF">
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>
                                            </a>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        <form action="{{ route('transaksi.order-masuk.selesai', $order->id_penjualan) }}" method="POST" class="inline">
                                            @csrf @method('PATCH')
                                            <button type="submit" class="px-3 py-1.5 text-xs font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-colors" title="Selesaikan Order (Stok akan dikurangi)" onclick="return confirm('Selesaikan order ini? Pastikan barang telah diserahkan, karena stok gudang akan dikurangi.')">
                                                Selesai
                                            </button>
                                        </form>
                                    </td>
