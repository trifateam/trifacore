                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-1">
                                            ...
                                            {{-- Proses / Selesai Buttons --}}
                                            @if($order->status_order === 'Menunggu')
                                                <form action="{{ route('transaksi.order-masuk.proses', $order->id_penjualan) }}" method="POST" class="inline">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" class="p-1.5 rounded-md text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50 dark:hover:bg-yellow-900/30 transition-colors" title="Proses Order" onclick="return confirm('Proses order ini?')">
                                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5.636 5.636a9 9 0 1012.728 0M12 3v9" /></svg>
                                                    </button>
                                                </form>
                                            @elseif($order->status_order === 'Diproses')
                                                <form action="{{ route('transaksi.order-masuk.selesai', $order->id_penjualan) }}" method="POST" class="inline">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" class="p-1.5 rounded-md text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 transition-colors" title="Selesaikan Order (Stok akan dikurangi)" onclick="return confirm('Selesaikan order ini? Stok gudang akan dikurangi.')">
