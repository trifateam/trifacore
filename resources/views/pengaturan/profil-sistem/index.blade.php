    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <x-card title="Konfigurasi Sistem">
                <form action="{{ route('pengaturan.profil-sistem.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <x-form-section title="Identitas Peternakan">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Logo Peternakan (Opsional)</label>
                            
                            <div class="flex items-center space-x-4">
                                @if(setting('logo_path') && Storage::disk('public')->exists(setting('logo_path')))
                                    <div class="relative w-20 h-20 rounded border border-gray-200 overflow-hidden flex-shrink-0 bg-gray-50" id="current-logo-container">
                                        <img src="{{ Storage::url(setting('logo_path')) }}" alt="Logo" class="w-full h-full object-contain" id="preview-image">
                                    </div>
                                @else
                                    <div class="relative w-20 h-20 rounded border border-gray-200 flex items-center justify-center bg-gray-50 flex-shrink-0" id="current-logo-container">
                                ...
                                <div class="flex-1">
                                    <input type="file" name="logo" id="logo-input" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" accept="image/jpeg,image/png">
                                    <p class="text-xs text-gray-500 mt-1">Format: JPG, PNG. Maksimal: 2MB.</p>
                                    
                                    <div class="mt-2 flex items-center">
                                        <input type="checkbox" name="remove_logo" id="remove_logo" value="1" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                        <label for="remove_logo" class="ml-2 text-sm text-red-600">Hapus Logo Saat Ini</label>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div>
            <x-card title="Preview Kop Surat PDF">
                ...
                <div class="border border-gray-300 p-4 bg-white rounded flex items-center justify-between" style="font-family: Arial, sans-serif; min-height: 120px;">
                    @php
                        $previewLogo = null;
                        if(setting('logo_path') && Storage::disk('public')->exists(setting('logo_path'))) {
                    ...
                    <div class="text-center" style="width: 60%;">
                        <h1 class="font-bold text-lg m-0 uppercase text-gray-800" id="pdf-preview-name">{{ setting('nama_peternakan') ?? 'NAMA PETERNAKAN' }}</h1>
                        <p class="text-xs m-0 text-gray-600 leading-snug mt-1" id="pdf-preview-address">
                            {!! nl2br(e(setting('alamat'))) ?? 'Alamat Peternakan' !!}
                        </p>
                        <p class="text-xs m-0 text-gray-600 leading-snug mt-1">
