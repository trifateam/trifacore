    ...
    <x-card class="mb-6 p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
        <form method="GET" action="{{ route('pencatatan.riwayat.deplesi') }}" class="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div class="w-full md:w-auto flex gap-2">
                ...
                <a href="{{ route('pencatatan.riwayat.deplesi') }}"><x-button variant="secondary" type="button">Reset</x-button></a>
            </div>
        </form>
    </x-card>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($records as $item)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-mono text-gray-900 dark:text-gray-100">{{ $item->kode_deplesi }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $item->tanggal_deplesi->translatedFormat('d M Y') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-10">
                                <x-empty-state message="Tidak ada data riwayat deplesi." icon="document" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
