...
@section('content')
    ...
    <x-page-header title="Riwayat Konsumsi Pakan" subtitle="Data riwayat lengkap pencatatan konsumsi pakan harian" />

    {{-- Filter Bar --}}
    <x-card class="mb-6 p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
        <form method="GET" action="{{ route('pencatatan.riwayat.konsumsi-pakan') }}" class="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div class="w-full md:w-auto flex gap-2">
                ...
                <a href="{{ route('pencatatan.riwayat.konsumsi-pakan') }}"><x-button variant="secondary" type="button">Reset</x-button></a>
            </div>
        </form>
    </x-card>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nama Pakan</th>
                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Jumlah (kg)</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($records as $item)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-mono text-gray-900 dark:text-gray-100">{{ $item->kode_pakan }}</td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">{{ number_format($item->jumlah_pakan_kg, 2) }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-10">
                                <x-empty-state message="Tidak ada data riwayat konsumsi pakan." icon="document" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
