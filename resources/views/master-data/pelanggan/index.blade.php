    ...
    @if($pelanggans->count() > 0)
        <x-table :headers="['No', 'Nama Pelanggan', 'Kategori', 'Kontak', 'Alamat', 'Status', 'Aksi']">
            @foreach($pelanggans as $index => $pelanggan)
                <tr>
                    <td class="px-4 py-3 text-sm text-gray-700">{{ $pelanggans->firstItem() + $index }}</td>
                    <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $pelanggan->nama_lengkap }}</td>
                    ...
                    <td class="px-4 py-3 text-sm text-gray-700">{{ $pelanggan->kontak }}</td>
                    <td class="px-4 py-3 text-sm text-gray-700 max-w-xs truncate" title="{{ $pelanggan->alamat }}">{{ Str::limit($pelanggan->alamat, 40) }}</td>
                    <td class="px-4 py-3 text-sm">
    ...
    <x-modal id="edit-pelanggan" title="Edit Data Pelanggan" size="xl">
        <form id="formEditPelanggan" method="POST" action="">
            ...
            <x-form-section title="Informasi Pelanggan" description="Perbarui data pelanggan">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                    <div class="mb-4">
                        <label for="edit_nama_lengkap" class="block text-sm font-medium text-gray-700 mb-1">Nama Pelanggan <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_lengkap" id="edit_nama_lengkap" required maxlength="100" placeholder="Contoh: CV Telur Makmur" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        <p class="mt-1 text-sm text-gray-500">Maksimal 100 karakter, harus unik</p>
                    </div>
                    <div class="mb-4">
                        <label for="edit_kontak" class="block text-sm font-medium text-gray-700 mb-1">No. Telp / Kontak <span class="text-red-500">*</span></label>
                        <input type="text" name="kontak" id="edit_kontak" required maxlength="20" placeholder="Contoh: 08123456789" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="kategori" id="edit_kategori" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="alamat" id="edit_alamat" required rows="3" placeholder="Masukkan alamat lengkap pelanggan..." class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"></textarea>
                    <p class="mt-1 text-sm text-gray-500">Alamat lengkap pelanggan</p>
                </div>
                <div class="flex items-center space-x-8 mt-2" x-data="{ aktif: true }" x-ref="editToggle">
                    <div class="flex items-center">
                        ...
                        <button type="button" role="switch" :aria-checked="aktif.toString()" @click="aktif = !aktif"
                            ...
                            <span aria-hidden="true" class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out" :class="aktif ? 'translate-x-5' : 'translate-x-0'"></span>
                        </button>
                        <label class="ml-3 block text-sm font-medium text-gray-700 cursor-pointer" @click="aktif = !aktif">Aktif</label>
                    </div>
                </div>
            </x-form-section>
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
