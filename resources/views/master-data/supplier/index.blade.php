    ...
    @if($suppliers->count() > 0)
        <x-table :headers="['No', 'Nama Supplier', 'Kontak', 'Alamat', 'PIC', 'Email', 'Aksi']">
            @foreach($suppliers as $index => $supplier)
                <tr>
                    <td class="px-4 py-3 text-sm text-gray-700">{{ $suppliers->firstItem() + $index }}</td>
                    <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $supplier->nama_supplier }}</td>
                    <td class="px-4 py-3 text-sm text-gray-700">{{ $supplier->kontak_supplier }}</td>
                    <td class="px-4 py-3 text-sm text-gray-700 max-w-xs truncate" title="{{ $supplier->alamat_supplier }}">{{ Str::limit($supplier->alamat_supplier, 40) }}</td>
                    <td class="px-4 py-3 text-sm text-gray-700">{{ $supplier->nama_pic ?? '-' }}</td>
    ...
    <x-modal id="edit-supplier" title="Edit Data Supplier" size="xl">
        <form id="formEditSupplier" method="POST" action="">
            ...
            <x-form-section title="Informasi Supplier" description="Perbarui data supplier">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                    <div class="mb-4">
                        <label for="edit_nama_supplier" class="block text-sm font-medium text-gray-700 mb-1">Nama Supplier <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_supplier" id="edit_nama_supplier" required maxlength="100" placeholder="Contoh: PT Pakan Sejahtera" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        <p class="mt-1 text-sm text-gray-500">Maksimal 100 karakter, harus unik</p>
                    </div>
                    <div class="mb-4">
                        <label for="edit_kontak_supplier" class="block text-sm font-medium text-gray-700 mb-1">No. Telp / Kontak <span class="text-red-500">*</span></label>
                        <input type="text" name="kontak_supplier" id="edit_kontak_supplier" required maxlength="20" placeholder="Contoh: 08123456789" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        <p class="mt-1 text-sm text-gray-500">Maksimal 20 karakter</p>
                    </div>
                    <div class="mb-4">
                        <label for="edit_email" class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                        <input type="email" name="email" id="edit_email" maxlength="100" placeholder="Contoh: supplier@email.com" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        <p class="mt-1 text-sm text-gray-500">Opsional, format email valid</p>
                    </div>
                    <div class="mb-4">
                        <label for="edit_nama_pic" class="block text-sm font-medium text-gray-700 mb-1">Nama PIC (Penanggung Jawab)</label>
                        <input type="text" name="nama_pic" id="edit_nama_pic" maxlength="100" placeholder="Contoh: Budi Santoso" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        <p class="mt-1 text-sm text-gray-500">Opsional, maksimal 100 karakter</p>
                    </div>
                </div>
                <div class="mb-4">
                    <label for="edit_alamat_supplier" class="block text-sm font-medium text-gray-700 mb-1">Alamat <span class="text-red-500">*</span></label>
                    <textarea name="alamat_supplier" id="edit_alamat_supplier" required rows="3" placeholder="Masukkan alamat lengkap supplier..." class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"></textarea>
                    <p class="mt-1 text-sm text-gray-500">Alamat lengkap supplier</p>
                </div>
            </x-form-section>
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
